    ...
    <tr data-table-row class="hover:bg-blue-50 transition border-b border-gray-100">
        ...
        <td class="px-4 py-3 text-center no-export whitespace-nowrap">
            <div class="inline-flex items-center justify-center gap-1">
                @can('difusions.edit')
                <a href="{{ url('/difusions/' . $accion->id . '/edit') }}"
                ...
                @endcan
                @can('difusions.destroy')
                <form action="{{ route('difusions.destroy', $accion->id) }}" method="POST" onsubmit="return confirm('¿Está seguro de eliminar esta acción?');" class="inline-flex items-center justify-center gap-1 m-0">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
