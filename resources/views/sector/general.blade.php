    ...
    <div class="col-12">
        <div class="stats-card">
            ...
            <div class="stats-info">
                <span class="stats-number" id="{{ $tableId }}-total">{{ number_format($sectores->total()) }}</span>
                <span class="stats-title">Total de Sectores del Aula ({{ $anio }})</span>
            </div>
        </div>
    </div>
</div>
        ...
        @if($showFullFilters)
            ...
            <div x-data="{
                    ...
                    async loadInstituciones() {
                        ...
                        try {
                            const res = await fetch('{{ route('buscarInstitucionporUgel-sec') }}?' + params.toString(), {
                    ...
                    async loadDocentes() {
                        ...
                        try {
                            const res = await fetch('{{ route('buscarDocenteporInstitucion-sec') }}?' + params.toString(), {
                                headers: { 'X-Requested-With': 'XMLHttpRequest' },
                            });
                            const data = await res.json();
