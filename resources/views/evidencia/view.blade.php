...
@section('title', 'Asistencia Técnica (General)')
...
@section('content_header')
    <h1 class="m-0 text-dark"><i data-lucide="file-text" class="w-6 h-6 mr-2 inline-block align-text-bottom"></i>Asistencia Técnica (General)</h1>
@stop
    ...
    <div class="col-12">
        <div class="stats-card">
            ...
            <div class="stats-info">
                <span class="stats-number" id="tabla-evidencias-general-total">{{ number_format($evidencias->total()) }}</span>
                <span class="stats-title">Total de Asistencia Técnica ({{ $anio }})</span>
            </div>
        </div>
    </div>
</div>
...
<x-table-base id="tabla-evidencias-general"
              ...
              :exportable="true"
              :searchable="true"
              exportFilename="asistencia_tecnica_general"
              :exportUrl="route('exportar.evidencias', request()->all())"
              :serverPaginated="true"
              :totalServerRecords="$evidencias->total()"
              :fromServer="$evidencias->firstItem() ?? 0"
              :toServer="$evidencias->lastItem() ?? 0"
              :filterAction="route('evidencias.view')">
    <x-slot name="filters">
        ...
        <div x-data="{
                ...
                async loadInstituciones() {
                    ...
                    try {
                        const res = await fetch('{{ route('buscarInstitucionporUgel-evi') }}?' + params.toString(), {
                ...
                async loadDocentes() {
                    ...
                    try {
                        const res = await fetch('{{ route('buscarDocenteporInstitucion-evi') }}?' + params.toString(), {
                            headers: { 'X-Requested-With': 'XMLHttpRequest' },
                        });
                        const data = await res.json();
...
<div id="tabla-evidencias-general-pagination" class="mt-3 flex justify-center sm:justify-end">
