...
const validationSchema = computed(() => yup.object({
    nombrePlan: yup
        .string()
        .trim()
        .required('El nombre del espacio de lectura es obligatorio.')
...
const [nombrePlan, nombrePlanAttrs] = defineField('nombrePlan', {
...
watch(() => props.show, (newVal) => {
    if (newVal) {
        ...
        if (props.plan) {
            resetForm({
                values: {
                    nombrePlan: props.plan.nombrePlan || '',
                    descripcion: props.plan.descripcion || '',
                    fecha: props.plan.fecha ? props.plan.fecha.split(' ')[0] : '',
                },
            });
        } else {
            resetForm({
...
const submit = handleSubmit((values) => {
    ...
    formData.append('nombrePlan', values.nombrePlan);
...
<template>
    <BaseModal
        :show="show"
        :title="isEditing ? 'Editar Espacio de Lectura en el Hogar' : 'Nuevo Espacio de Lectura en el Hogar'"
        :subtitle="isEditing ? 'Modifique los campos necesarios para actualizar el registro' : 'Complete los campos para registrar un nuevo espacio de lectura en el hogar'"
        icon="BookHeart"
        color="cyan"
        max-width="2xl"
    ...
    >
        <div class="space-y-4">
            <div>
                <label for="nombrePlan" class="block text-xs font-bold text-slate-700 mb-1">
                    Nombre del Espacio de Lectura <span class="text-rose-500">*</span>
                </label>
                <div class="relative">
                    <input
                        id="nombrePlan"
                        ...
                        :class="errors.nombrePlan ? 'border-rose-400 bg-rose-50/20 text-rose-900 focus:ring-rose-400 focus:border-rose-400' : 'border-slate-300'"
                    />
                </div>
                <p v-if="errors.nombrePlan" class="mt-1 text-xs text-rose-600 font-semibold flex items-center">
            ...
            <div>
                ...
                <textarea
                    ...
                    placeholder="Ingrese detalles o aspectos relevantes del espacio de lectura..."
                    class="w-full text-xs bg-white border rounded-xl px-3 py-2.5 text-slate-800 transition focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <div>
                ...
                <div v-if="isEditing && plan.enlace && !selectedFileName" class="mb-2 p-2.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
                    <div class="flex items-center min-w-0">
                        <Paperclip class="w-4 h-4 text-slate-500 mr-2 shrink-0" />
                        <span class="text-xs text-slate-700 font-medium truncate">
                            Archivo actual: <strong class="text-slate-900">{{ plan.enlace.split('/').pop() }}</strong>
                        </span>
                    </div>
