...
// UGEL → Institución → Docente encadenados por AJAX (igual que el Blade): el
// filtro del servidor exige coincidencia exacta y las listas completas serían
// demasiado pesadas para el prop.
...
async function loadInstituciones() {
    ...
    try {
        ...
        const res = await fetch(`/buscar-instituciones-por-ugel-plan?${params.toString()}`, {
...
async function loadDocentes() {
    ...
    try {
        ...
        const res = await fetch(`/buscar-docentes-por-institucion-pla?${params.toString()}`, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' },
        });
        const data = await res.json();
...
const textoLabel = computed(() => (isDirector.value ? 'Nombre del Espacio de Lectura' : 'DNI del Docente'));
const textoPlaceholder = computed(() => (isDirector.value ? 'Ej. Mi rincón de lectura' : 'Ingrese DNI'));
...
<template>
    <AppLayout>
        <Head title="Espacio de Lectura en el Hogar - Consulta" />
