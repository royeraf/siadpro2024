...
const isEditing = computed(() => !!props.sector?.id);
...
const [nombreSector, nombreSectorAttrs] = defineField('nombreSector', {
...
watch(() => props.show, (newVal) => {
    if (newVal) {
        ...
        if (props.sector) {
            resetForm({
                values: {
                    nombreSector: props.sector.nombreSector || '',
                    descripcion: props.sector.descripcion || '',
                    fecha: props.sector.fecha ? props.sector.fecha.split(' ')[0] : '',
                },
            });
        } else {
            resetForm({
...
const submit = handleSubmit((values) => {
    ...
    formData.append('nombreSector', values.nombreSector);
...
<template>
    <BaseModal
        :show="show"
        :title="isEditing ? 'Editar Sector del Aula' : 'Nuevo Sector del Aula'"
        :subtitle="isEditing ? 'Modifique los campos necesarios para actualizar el registro' : 'Complete los campos para registrar un nuevo sector del aula'"
        icon="LayoutGrid"
        color="gray"
        max-width="2xl"
    ...
    >
        <div class="space-y-4">
            <div>
                <label for="nombreSector" class="block text-xs font-bold text-slate-700 mb-1">
                    Nombre del Sector <span class="text-rose-500">*</span>
                </label>
                <div class="relative">
                    <input
                        id="nombreSector"
                        ...
                        v-bind="nombreSectorAttrs"
                        placeholder="Ej. Sector de Lectura Comprensiva"
                        ...
                        :class="errors.nombreSector ? 'border-rose-400 bg-rose-50/20 text-rose-900 focus:ring-rose-400 focus:border-rose-400' : 'border-slate-300'"
                    />
                </div>
                <p v-if="errors.nombreSector" class="mt-1 text-xs text-rose-600 font-semibold flex items-center">
            ...
            <div>
                ...
                <textarea
                    ...
                    placeholder="Ingrese detalles o aspectos relevantes del sector del aula..."
            ...
            <div>
                ...
                <div v-if="isEditing && sector.enlace && !selectedFileName" class="mb-2 p-2.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
                    <div class="flex items-center min-w-0">
                        <Paperclip class="w-4 h-4 text-slate-500 mr-2 shrink-0" />
                        <span class="text-xs text-slate-700 font-medium truncate">
                            Archivo actual: <strong class="text-slate-900">{{ sector.enlace.split('/').pop() }}</strong>
                        </span>
                    </div>
