...
import SectorFormModal from './Partials/SectorFormModal.vue';
...
function openCreateModal() {
    formModal.sector = null;
    formModal.show = true;
}

function openEditModal(sector) {
    formModal.sector = sector;
    formModal.show = true;
}
...
function handleExportExcel() {
    const dataToExport = (props.sectores.data || []).map((row, idx) => ({
        '#': idx + 1,
        'Nombre del Sector': row.nombreSector,
    ...
    XLSX.writeFile(wb, `sectores_aula_${new Date().toISOString().slice(0, 10)}.xlsx`);
}

function confirmDelete(sector) {
    Swal.fire({
        title: '¿Está seguro de eliminar este sector?',
        text: `Se eliminará "${sector.nombreSector}".`,
        icon: 'warning',
...
<template>
    <AppLayout>
        ...
        >
            <template #filters>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Nombre del Sector</label>
                    <input
                        type="text"
                        v-model="localFilters.texto"
                        placeholder="Ej. Sector de Lectura"
