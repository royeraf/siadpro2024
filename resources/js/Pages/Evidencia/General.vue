...
watch(() => props.filters, (f) => {
    localFilters.anio = f.anio || props.anio || String(new Date().getFullYear());
...
const textoLabel = computed(() => (isDirector.value ? 'Nombre de la Asistencia' : 'DNI del Docente'));
const textoPlaceholder = computed(() => (isDirector.value ? 'Ej. Taller de capacitación' : 'Ingrese DNI'));
...
<template>
    <AppLayout>
        <Head title="Asistencia Técnica - Consulta" />
        ...
        <BaseTable
            ...
            :export-filename="`asistencia_tecnica_${scope}`"
            :loading="loading"
            @search="handleSearch"
