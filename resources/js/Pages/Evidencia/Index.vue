...
import EvidenciaFormModal from './Partials/EvidenciaFormModal.vue';
...
const columns = computed(() => {
    const cols = [
        { key: 'nombreEvidencia', label: 'Nombre de la Asistencia', sortable: false },
...
function openCreateModal() {
    formModal.evidencia = null;
    formModal.show = true;
}

function openEditModal(evidencia) {
    formModal.evidencia = evidencia;
    formModal.show = true;
}
...
function openViewer(evidencia) {
    if (!evidencia.enlace) return;
    const fileName = evidencia.enlace.split('/').pop() || 'documento';
    viewer.name = fileName;
    viewer.url = `/visor/stream?path=${encodeURIComponent(evidencia.enlace)}`;
    viewer.downloadUrl = `/evidencias/${evidencia.id}/download`;
    viewer.show = true;
}
...
function handleExportExcel() {
    const dataToExport = (props.evidencias.data || []).map((row, idx) => ({
        '#': idx + 1,
        'Nombre de la Asistencia': row.nombreEvidencia,
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Asistencia Técnica');
    XLSX.writeFile(wb, `asistencia_tecnica_${new Date().toISOString().slice(0, 10)}.xlsx`);
}

function confirmDelete(evidencia) {
    Swal.fire({
        title: '¿Está seguro de eliminar esta asistencia técnica?',
        text: `Se eliminará "${evidencia.nombreEvidencia}".`,
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            router.delete(`/evidencias/${evidencia.id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire({
                        title: '¡Eliminado!',
...
<template>
    <AppLayout>
        ...
        >
            <template #filters>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Nombre de la Evidencia</label>
                    <input
                        type="text"
