...
const isEditing = computed(() => !!props.evidencia?.id);
...
const validationSchema = computed(() => yup.object({
    nombreEvidencia: yup
        .string()
        .trim()
        .required('El nombre de la asistencia técnica es obligatorio.')
...
const [nombreEvidencia, nombreEvidenciaAttrs] = defineField('nombreEvidencia', {
...
watch(() => props.show, (newVal) => {
    if (newVal) {
        ...
        if (props.evidencia) {
            resetForm({
                values: {
                    nombreEvidencia: props.evidencia.nombreEvidencia || '',
                    descripcion: props.evidencia.descripcion || '',
                    fecha: props.evidencia.fecha ? props.evidencia.fecha.split(' ')[0] : '',
                },
            });
        } else {
            resetForm({
...
const submit = handleSubmit((values) => {
    ...
    formData.append('nombreEvidencia', values.nombreEvidencia);
...
<template>
    <BaseModal
        :show="show"
        :title="isEditing ? 'Editar Asistencia Técnica' : 'Nueva Asistencia Técnica'"
        :subtitle="isEditing ? 'Modifique los campos necesarios para actualizar el registro' : 'Complete los campos para registrar una nueva asistencia técnica'"
        icon="FileText"
        color="red"
        max-width="2xl"
    ...
    >
        <div class="space-y-4">
            <div>
                <label for="nombreEvidencia" class="block text-xs font-bold text-slate-700 mb-1">
                    Nombre de la Asistencia <span class="text-rose-500">*</span>
                </label>
                <div class="relative">
                    <input
                        id="nombreEvidencia"
                        ...
                        v-bind="nombreEvidenciaAttrs"
                        placeholder="Ej. Visita de acompañamiento"
                        ...
                        :class="errors.nombreEvidencia ? 'border-rose-400 bg-rose-50/20 text-rose-900 focus:ring-rose-400 focus:border-rose-400' : 'border-slate-300'"
                    />
                </div>
                <p v-if="errors.nombreEvidencia" class="mt-1 text-xs text-rose-600 font-semibold flex items-center">
                    <AlertCircle class="w-3.5 h-3.5 mr-1 shrink-0" />
                    {{ errors.nombreEvidencia }}
                </p>
            </div>
            ...
            <div>
                ...
                <textarea
                    ...
                    placeholder="Ingrese detalles o aspectos relevantes de la asistencia técnica..."
            ...
            <div>
                ...
                <div v-if="isEditing && evidencia.enlace && !selectedFileName" class="mb-2 p-2.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
                    <div class="flex items-center min-w-0">
                        <Paperclip class="w-4 h-4 text-slate-500 mr-2 shrink-0" />
                        <span class="text-xs text-slate-700 font-medium truncate">
                            Archivo actual: <strong class="text-slate-900">{{ evidencia.enlace.split('/').pop() }}</strong>
                        </span>
                    </div>
